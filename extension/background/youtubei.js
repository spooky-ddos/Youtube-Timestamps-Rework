...
export async function fetchComments(videoId) {
    ...
    // Usuwamy limity i kontynuujemy aż nie będzie więcej tokenów
        ...
        try {
            ...
            for (const item of items) {
                if (item.commentThreadRenderer) {
                    ...
                    // Aktualizujemy token do następnej strony
                    token = item.continuationItemRenderer.continuationEndpoint.continuationCommand.token
                }
            }
            pageCount++
            
            // Dodajemy małe opóźnienie między zapytaniami aby nie przeciążyć API
    ...
    console.log(`Fetched ${comments.length} comments from ${pageCount} pages`)
    return comments
}
